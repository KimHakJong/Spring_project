       ...
       $.ajax({
		   ...
		   success : function(data){
			...
			if(data.listcount > 0){ // 총 갯수가 0보다 큰 경우
			   ...
			   console.log(num);
			   let output = "<tbody>"
			   $(data.boardlist).each( // 받아온 data.boardlist 형태가 배열이기 때문에 each사용 
				    function(index,item) {
						output += '<tr><td>' + (num--) + '</td>'
